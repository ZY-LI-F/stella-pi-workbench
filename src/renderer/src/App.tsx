...
function LoadingScreen({ skin }: { readonly skin: SkinPreference }) {
  return (
...
export function App({ api }: AppProps) {
  ...
  const paletteActions = useMemo<readonly PaletteAction[]>(
    ...
    [state.bootstrap?.state.sessionId],
  );
  ...
  if (state.phase === "error" || !state.bootstrap) {
    return (
      <div className="startup-screen startup-screen--error">
        ...
        <Brand skin={preferences.skin} />
        <span className="startup-screen__error-mark">!</span>
        <h1>Pi 没有连接成功</h1>
        <p>{state.error || "初始化没有返回工作区状态。"}</p>
        {state.stderr && <details><summary>查看 Pi 诊断输出</summary><pre>{state.stderr}</pre></details>}
        <button type="button" className="button-primary" onClick={() => void chooseProject()}><FolderOpen size={15} />选择其他项目</button>
  ...
  return (
    <div className={`app-shell app-shell--${workspaceView} ${workspaceView === "chat" && inspectorOpen ? "has-inspector" : ""} ${terminalOpen ? "has-terminal" : ""}`}>
      ...
      {renameOpen && <TextPromptDialog title="重命名会话" eyebrow="SESSION NAME" label="会话名称" initialValue={bootstrap.state.sessionName ?? ""} confirmLabel="保存名称" onCancel={() => setRenameOpen(false)} onConfirm={(name) => {
        setRenameOpen(false);
