...
- **业务阶段与执行真相分离**：Task 的看板阶段由用户维护；Workflow Run / AgentTask 独立显示排队、运行、等待、已报告、失败或中断。Agent 返回结果只会成为 `reported + pending acceptance`，不会伪装成已完成。
- **明确验收**：用户可以接受、请求修订或拒绝一次报告；决定、说明和时间会进入不可变消息与活动记录，但不会偷偷移动 Task 卡片。
...
看板交互包括：创建与编辑任务、项目/流程筛选、搜索、原生拖放、分发与重新分发、Task Room、只读 DAG、实时星轨进度、Agent 与工具事件、Markdown 产物、会话文件定位、人工批准/驳回、执行报告验收、中止流程、手动归档、删除，以及 Agent/团队/流程目录。看板列只表示长期业务阶段；队列和 Runtime 状态显示在独立执行标签中，不会由编排器偷偷覆盖 Task 阶段。用户手动拖放只允许进入“待规划、受阻、已完成”这些明确业务列。
...
- **评论与 `@mention` 委派**：Task Room 中的精确 `@builder` / `@BUILD` 会生成真实 AgentTask；提交前会显示将创建的任务数量和委派顺序。未知或歧义 mention 会整体拒绝，不留下半条消息或半组队列。
- **动态 Squad**：用户可保存 Leader、可委派成员与 Leader 固定指令。Leader 先由真实 Pi 执行，再依据产物中的精确 `@mention` 创建成员任务；全部子项报告后根项进入 `reported + pending acceptance`，任一成员失败会保存原因并终止同组剩余项。
...
看板状态存放在 Electron 的用户数据目录下 `board/board.json`，与被打开的代码仓库分离，因此不会向他人的项目写入 Stella 配置。旧 schema 升级时会先在同目录创建时间戳备份，再无损迁移到 v3。Agent 步骤使用接收者自己的 Pi 模型和认证；内置角色不硬编码 API Key、模型或本机 Pi 安装路径。
