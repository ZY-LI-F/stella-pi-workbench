...
- **一个 Team Chat 入口**：左侧“团队协作”不是另一套聊天数据库；左栏选择 Task Channel，中栏展开同一个 Task Room，右栏显示从执行记录派生的 Team Pulse。
...
- **`@lead` Coordinator**：`@lead` 启动通用调度负责人。LEAD 只能返回严格 JSON 行动：`delegate / request_revision / replan / complete / ask_human`；Stella 验证字段和 Agent 范围后才创建真实子任务。成员报告后会再启动一个 LEAD 验收回合，而不是直接把父任务判为完成；`ask_human` 会停在 Task Room，普通用户回复会唤醒下一回合。自然语言里的 `@mention` 不会被当成 LEAD 已委派。
...
- 团队协作：左侧 Task Channel、中间完整 Task Room、右侧 Team Pulse；支持 `@lead`、直接 `@worker`、LEAD 追问恢复和项目 AgentDraft。
