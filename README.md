...
- **Provider 连接状态**：区分 `auth.json`、环境变量、`models.json` 内联配置与 OAuth；页面只返回状态元数据，不读取或回显已有 API Key/OAuth token。
- **API Key 管理**：新密钥通过 preload 的窄 IPC 一次性送到主进程，写入 Pi 标准 `auth.json`，文件权限限制为当前用户。替换密钥时保留 Provider 专属 `env` 配置；也可以显式清除已保存凭据。
...
- **明确应用语义**：保存后重载真实 Pi RPC，并通过当前 `sessionFile` 恢复同一会话；配置或重载失败会原样显示，不伪造成功。OAuth/订阅登录仍由 Pi 的交互式 `/login <provider>` 流程负责。
...
- 全局模型、Provider 配置与思考级别：左侧 `MODEL RELAY` 在全部页面始终显示同一个 Pi 当前模型；独立“模型配置”页可检查鉴权来源、安全写入/清除 `auth.json` 凭据、维护 `models.json` 自定义 Provider/模型并从 Pi 真实可用目录切换全局路由。未声明模型覆盖的 Agent、LEAD、Squad 和 Workflow 步骤继承该选择，项目 Agent 的显式 Provider/Model 设置优先；会话页支持 `off` 到 `max` 的完整思考级别。
...
```text
71AF115D666CDDA3B166ACE7543E3DF74EABE320796F34EE3CA7099B3B458ACA
```
