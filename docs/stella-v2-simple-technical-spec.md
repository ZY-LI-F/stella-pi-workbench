...
Stella v2 保留当前 Electron、React、typed preload、Pi RPC、JSON 原子写入和完整 Board Snapshot 推送模式，只做四项结构调整：
...
最终产品仍然只有一个桌面进程、一个本地状态文件、一套 Pi Runtime 和一个确定性 DAG 编排器。
...
Main Process
  ...
  -> PiRpcRuntime           Bundled Pi RPC child process
```
